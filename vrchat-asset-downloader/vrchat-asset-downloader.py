...
HEADERS = {"Host": "vrchat.com",
           "User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:91.0) Gecko/20100101 Firefox/91.0"}
GUID_REGEX = r"_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}"
ASSET_REGEX = r"^(?P<asset_type>wrld)" + GUID_REGEX + r"$"
FILE_REGEX = r"^https?:\/\/api\.vrchat\.cloud\/api\/1\/file\/(?P<file_id>file" + \
    GUID_REGEX + r")\/[0-9]+\/file$"
CLEAN_FILENAME_WIN = r"[\/\\:*?\"<>|]"
CLEAN_FILENAME_POS = r"[\/]"
BLOCK_SIZE = 1024
REMOVE_FROM_JSON = ["favorites", "visits", "popularity", "heat",
                    "publicOccupants", "privateOccupants", "occupants", "instances"]
...
def get_auth(s):
    ...
    if not config_r.ok:
        ...
        print_log(
            "config", f"config endpoint returned status '{config_r.status_code}'", level=LogLevel.VERBOSE)
        sys.exit(1)
    config_j = config_r.json()
    if "clientApiKey" not in config_j or not config_j["clientApiKey"]:
        print_log("config", "failed to retrieve API key")
        print_log("config", f"config response lacks clientApiKey value",
    ...
    if not auth_cookie or len(auth_cookie) == 0:
        print_log(
            "auth", f"while logged in to vrchat.com, visit '{API_URL}auth?apiKey={clientKey}' in your browser")
    ...
    if not auth_r.ok:
        print_log(
            "auth", "error: the token you provided does not appear to be valid")
        sys.exit(1)
...
def download_asset(a_type, a_id, s, api_key):
    ...
    if not r.ok:
        ...
        print_log(
            f"{a_type}", f"asset endpoint returned status '{r.status_code}'", level=LogLevel.VERBOSE)
        return
    asset_j = r.json()
    ...
    if "assetUrl" in asset_j and asset_j["assetUrl"]:
        ...
        if asset_m:
            ...
            print_log(
                f"{a_type}", f"found asset for '{asset_j['name']}' ({a_id})")
            file_j = get_file_json(file_id, s)
            asset_j["_assetFile"] = file_j
        else:
            print_log(
                f"{a_type}", f"could not find the asset url for '{asset_j['name']}' ({a_id})")
            print_log(
                f"{a_type}", f"assetUrl did not match expected pattern ('asset_j['assetUrl']')", level=LogLevel.VERBOSE)
        ...
        print_log(
            f"{a_type}", f"could not find the asset url for '{asset_j['name']}' ({a_id})")
        print_log(f"{a_type}", f"asset response lacks assetUrl value",
                  level=LogLevel.VERBOSE)
    ...
    if "imageUrl" in asset_j and asset_j["imageUrl"]:
        ...
        if image_m:
            ...
            print_log(
                f"{a_type}", f"found image for '{asset_j['name']}' ({a_id})")
            image_j = get_file_json(image_id, s)
            asset_j["_imageFile"] = image_j
        else:
            print_log(
                f"{a_type}", f"could not find the image url for '{asset_j['name']}' ({a_id})")
            print_log(
                f"{a_type}", f"imageUrl did not match expected pattern ('{asset_j['imageUrl']}')", level=LogLevel.VERBOSE)
    else:
        print_log(
            f"{a_type}", f"could not find the image url for '{asset_j['name']}' ({a_id})")
        print_log(f"{a_type}", f"asset response lacks imageUrl value",
                  level=LogLevel.VERBOSE)
    ...
    if args.list_revisions:
        ...
        save_dir = os.path.join(
            args.directory, clean_filename(asset_j["name"]))
        ...
        if args.write_json:
            ...
            print_log(
                f"{a_type}", f"writing asset information to '{json_filename}'")
...
def get_file_json(f_id, s):
    ...
    if not r.ok:
        ...
        print_log(
            "file", f"file endpoint returned status '{r.status_code}'", level=LogLevel.VERBOSE)
        return None
    file_j = r.json()
    return file_j
...
def download_file_from_json(file_j, save_dir, s):
    ...
    if args.revisions == "all":
        ...
        print_log(
            "file", f"error: revision specified out of range, try --list-revisions")
        return
    else:
    ...
    for dl_num, dl_ver in enumerate(get_versions):
        ...
        if os.path.isfile(file_path):
            ...
            redirect_r = s.get(
                cur_j["url"], stream=True, allow_redirects=False)
            if not redirect_r.ok:
                print_log(
                    f"file", f"could not retrieve file for '{file_j['id']}'")
                print_log(
                    f"file", f"file url '{cur_j['url']}' returned status '{redirect_r.status_code}'", level=LogLevel.VERBOSE)
                break
            ...
            if not file_r.ok:
                print_log(
                    f"file", f"could not retrieve file for '{file_j['id']}'")
                print_log(
                    f"file", f"file url '{cur_j['url']}' returned status '{file_r.status_code}'", level=LogLevel.VERBOSE)
                break
...
def get_arguments():
    ...
    parser.add_argument("--write-thumbnail", action="store_true",
                        help="save thumbnail for the asset (if used with '--revision all', all thumbnail revisions will be retrieved)")
    parser.add_argument("--write-json", action="store_true",
                        help="write metadata to .json file(s)")
    parser.add_argument("--dont-clean-json", action="store_false",
                        help="retain all json values when writing .json file(s)")
    parser.add_argument("--verify", action="store_true",
                        help="whether or not to verify downloaded files against remote hashes", default=False)
    parser.add_argument("--skip-download", action="store_true",
                        help="skip downloading the actual asset(s)")
    parser.add_argument("--revisions", type=str,
                        help="valid values are the keywords 'all' and 'latest', or the revision integer itself", default="latest")
    parser.add_argument("--list-revisions", action="store_true",
                        help="list available revisions for the specified asset")
    parser.add_argument("asset_id_list", metavar="ASSET IDS", nargs="*",
                        help="world/avatar id(s) i.e. wrld_12345678-90ab-cdef-1234-567890abcdef")
...
def main():
    ...
    for asset_id in args.asset_id_list:
        ...
        if asset_type_m:
            ...
            download_asset(ASSET_TYPES[asset_type],
                           asset_id, api_session, api_key_t)
        else:
            print_log("vrchat-asset-downloader",
                      f"id {asset_id} does not appear to be valid")
