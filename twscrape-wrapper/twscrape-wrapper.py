...
OPERATIONS = ["save", "save-past", "sort", "dedupe", "cookies-to-string"]
KEY_ORDER_TWEET = ["_type", "url", "date", "rawContent", "renderedContent", "id", "user", "replyCount", "retweetCount", "likeCount", "quoteCount", "conversationId", "lang", "source", "sourceUrl", "sourceLabel", "links", "media", "retweetedTweet",
                   "quotedTweet", "inReplyToTweetId", "inReplyToUser", "mentionedUsers", "coordinates", "place", "hashtags", "cashtags", "card", "viewCount", "vibe", "content", "outlinks", "outlinksss", "tcooutlinks", "tcooutlinksss", "username"]
KEY_ORDER_USER = ["_type", "username", "id", "displayname", "rawDescription", "renderedDescription", "descriptionLinks", "verified", "created", "followersCount", "friendsCount", "statusesCount",
                  "favouritesCount", "listedCount", "mediaCount", "location", "protected", "link", "profileImageUrl", "profileBannerUrl", "label", "description", "descriptionUrls", "linkTcourl", "linkUrl", "url"]
BACKWARDS_INTERVAL = 120
MEDIA_HEADERS = {
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:128.0) Gecko/20100101 Firefox/128.0"}
MEDIA_IMAGE_NAMES = ["orig", "large", "medium", "900x900", "small", "thumb"]
MEDIA_SLEEP_BETWEEN_DOWNLOADS = 2
...
def get_last_tweet(tweets, since=True):
    if len(tweets) > 0:
        last_tweet_date = datetime.datetime.fromisoformat(
            tweets[0]["date"]).timestamp()
    else:
        last_tweet_date = -1
    last_tweet_id = None
    for tweet in tweets:
        current_tweet_date = datetime.datetime.fromisoformat(
...
async def gather_initial_tweets(api, account_handle):
    ...
    for tweet in user_tweets:
        if tweet.user.username == account_handle:
            tweets.append(json.loads(json.dumps(
                tweet.dict(), default=datetime_handler)))
    return tweets
...
async def gather_tweets_backwards(api, account_handle, latest_timestamp):
    ...
    while True:
        ...
        back_one_month = latest_datetime - \
            datetime.timedelta(days=BACKWARDS_INTERVAL)
...
def order_tweet_dict(tweet):
    ordered_tweet = OrderedDict((key, tweet.get(key))
                                for key in KEY_ORDER_TWEET)
    ordered_tweet["user"] = OrderedDict(
        (key, ordered_tweet["user"].get(key)) for key in KEY_ORDER_USER)
...
def write_tweets(tweets, tmp_filepath, filepath, overwrite):
    with open(tmp_filepath, "w") as tmp_tweetsfile:
        for tweet in tweets:
            tweet_json = json.dumps(order_tweet_dict(
                tweet), default=datetime_handler)
    ...
    if not os.path.isdir(media_directory):
        try:
            ...
            try:
                ...
                print(
                    "could not find or make the media directory, skipping media downloads")
                return
    ...
    for tweet in tweets:
        if "media" in tweet and tweet["media"]:
            ...
            if "photos" in tweet_media and tweet_media["photos"]:
                ...
                for image in images:
                    image_url = image["url"]
                    image_url_path, image_ext = os.path.splitext(
                        image_url.split("?")[0])
                    ...
                    for img_name in MEDIA_IMAGE_NAMES:
                        ...
                        image_res = download_media_file(
                            tweet_url, image_url, os.path.join(media_directory, image_filename))
                        if image_res > 1:
                            media_count += 1
                        if image_res > 0:
                            media_index += 1
            ...
            if "videos" in tweet_media and tweet_media["videos"]:
                ...
                for video in videos:
                    video_variants_sorted = sorted(
                        video["variants"], key=lambda v: v["bitrate"], reverse=True)
                    for variant in video_variants_sorted:
                        video_url = variant["url"]
                        _, video_ext = os.path.splitext(
                            video_url.split("?")[0])
                        video_filename = f"{tweet_id}_{media_index}{video_ext}"
                        video_res = download_media_file(
                            tweet_url, video_url, os.path.join(media_directory, video_filename))
                        if video_res > 1:
                            media_count += 1
                        if video_res > 0:
                            media_index += 1
            ...
            if "animated" in tweet_media and tweet_media["animated"]:
                ...
                for animation in animations:
                    ...
                    _, animation_ext = os.path.splitext(
                        animation_url.split("?")[0])
                    animation_filename = f"{tweet_id}_{media_index}{animation_ext}"
                    animation_res = download_media_file(
                        tweet_url, animation_url, os.path.join(media_directory, animation_filename))
                    if animation_res > 1:
                        media_count += 1
                    if animation_res > 0:
                        media_index += 1
...
async def main():
    ...
    parser.add_argument("mode", choices=OPERATIONS,
                        help="operation to perform. 'save' downloads tweets to a file ('save-past' works in reverse), 'sort' re-orders tweets in a file, 'dedupe' removes entries with duplicate ids. 'cookies-to-string' prints a Netscape HTTP Cookie File in a format compatible for adding accounts.")
    parser.add_argument(
        "filename", help="file prefix to write tweets to (will be appended with .tweets.json)")
    parser.add_argument("handle", nargs="?",
                        help="handle of the account to download from")
    parser.add_argument("-n", action="store_false",
                        help="prompt for overwriting the existing tweet file")
    parser.add_argument("--download-media", "-m", action="store_true",
                        help="iterate through scraped/sorted tweets and download all media")
    args = parser.parse_args()
    if args.mode == OPERATIONS[4]:
        filepath = args.filename
        if not os.path.isfile(filepath):
            filepath = os.path.join(os.getcwd(), filepath)
        cookie_jar = cookiejar.MozillaCookieJar(filepath)
        try:
            cookie_jar.load()
            cookies_string = ""
            ...
            print("cookies could not be loaded, file format might not be valid")
        return
    ...
    if args.mode == OPERATIONS[0] or args.mode == OPERATIONS[1]:
        ...
        account_handle = args.handle if args.handle else get_account_from_file(
            filepath)
        ...
        last_saved_tweet_date, last_saved_tweet_id = get_last_tweet(
            saved_tweets, since)
        tweets_gathered = []
        if last_saved_tweet_date < 0:
            ...
            last_saved_datetime = datetime.datetime.fromtimestamp(
                last_saved_tweet_date)
            print(
                f"retrieving tweets {'since' if since else 'until'} {last_saved_datetime.isoformat()}")
            if since:
            ...
            tweets_gathered = list(filter(lambda t: datetime.datetime.fromisoformat(
                t["date"]).timestamp() != last_saved_tweet_date, tweets_gathered))
        if len(tweets_gathered) > 0:
            sorted_tweets_gathered = sort_tweets(tweets_gathered)
            last_retrieved_tweet_date = datetime.datetime.fromisoformat(
                sorted_tweets_gathered[-1]["date"]).timestamp()
            if last_saved_tweet_date > 0 and last_retrieved_tweet_date > last_saved_tweet_date:
                last_retrieved_datetime = datetime.datetime.fromtimestamp(
                    last_retrieved_tweet_date)
                tweets_difference = last_retrieved_datetime - last_saved_datetime
                print(
                    f"warning: oldest tweet retrieved is from {last_retrieved_datetime.isoformat()}, {tweets_difference.days} days difference")
            ...
            print(
                f"scraped {len(tweets_sorted) - len(saved_tweets)} new tweets")
        ...
        if len(saved_tweets) > 0:
            ...
            print(
                f"removed {len(saved_tweets) - len(tweets_filtered)} duplicates")
