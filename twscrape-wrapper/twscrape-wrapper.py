...
async def main():
    ...
    parser.add_argument("mode", choices=OPERATIONS, help="operation to perform. 'save' downloads tweets to a file, 'sort' re-orders tweets in a file.")
    ...
    if args.mode == OPERATIONS[0]:
        ...
        if len(tweets_gathered) > 0:
            ...
            print(f"scraped {len(sorted_tweets_gathered)} new tweets")
            sorted_tweets_gathered.extend(saved_tweets)
            tweets_sorted = sort_tweets(sorted_tweets_gathered)
